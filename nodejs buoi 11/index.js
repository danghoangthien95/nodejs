...
app.get('/xoa/:i', function (req, res) {
	// body...
	var i = req.params.i;
	arr.splice(i, 1); // xoa tu vi tri i va xoa 1 phan tu
...
app.get('/xoa/:i', function (req, res) {
	...
	fs.unlink(`./public/images/background/%(arr[i].images)`, (err) => {
		if (err) throw err;
...
app.get('/sua/:i', function (req, res) {
	...
	res.render('edit', {arr: arr[i], i: i});
})

app.post('/sua', parser, function (req, res) {
	// body...
	upload(req, res, function (err) {
		var titles = req.body.titles;
		var id = req.body.id;
		...
		var i = req.body.i;
		var images;
		// check underfined, n dung co gui hinh len
		if(req.file === undefined) {
			images = arr[i].images;
			var sp = new Video(id, titles, images, desc)
			arr[i] = sp;
			res.redirect('/danhsach');
		} else {
			images = req.file.filename;
			fs.unlink(`./public/images/background/%(arr[i].images)`,
				function() {
				// tao ra doi tuong san pham moi
				var sp = new Video(id, titles, images, desc)
				arr[i] = sp;
				res.redirect('/danhsach');
			});
